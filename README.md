...
日次 plan issue から Task issue を起票する際は、**`./PLAN.md` Section 10 の選定基準と優先順位ルール**を参照してください。

| 判断ポイント | 確認内容 |
| --- | --- |
| 未実装か | README / PLAN に ✅ がないテーマを選ぶ |
| 重複がないか | 同テーマの open Task issue が存在しないことを確認する |
| 1サイクルで完結するか | 数日以内に DoD を満たせる範囲に収める |
| 完了条件が書けるか | 開始前に DoD を明確に書けるテーマを選ぶ |
| 優先度の順序 | 運用の穴 → 品質改善 → Phase 2 定着 → 新機能 の順で優先する |
