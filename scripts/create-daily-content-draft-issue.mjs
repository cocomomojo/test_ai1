...
const title = `日次コンテンツ下書き ${planDate}`;
...
const issueBody = [
  ...
  "- 注意: 自動生成された下書き候補です。公開前に内容と著作権観点を確認してください。",
  "",
  generatedBody
].join("\n");
...
const tempDir = mkdtempSync(path.join(tmpdir(), "daily-content-draft-issue-"));
...
function buildPrompt({ repository: currentRepository, planDate: currentPlanDate, openIssues: currentOpenIssues, recentClosedIssues: currentClosedIssues }) {
  const promptTemplate = readWorkspaceFile(".github/prompts/daily-content-draft.prompt.md");
