...
const AUTO_GENERATED_DAILY_TITLE_PREFIXES = ["日次プラン提案", "日次コンテンツ下書き"];
const AUTO_GENERATED_DAILY_SOURCE_MARKERS = [
 ...
 * Excludes auto-generated daily issues, themes already completed in README/PLAN,
 * and topics that overlap with open/closed task issues.
 *
...
export function filterDailyPlanCandidateIssues(
  ...
  return issues.filter((issue) => {
    if (isAutoGeneratedDailyIssue(issue)) {
      return false;
    }
...
export function prioritizeDailyPlanCandidateIssues(candidates) {
  return candidates.filter((candidate) => !isAutoGeneratedDailyIssue(candidate)).sort((left, right) => {
...
/**
 * Detects whether an issue is an auto-generated daily plan/content-draft issue.
 * Uses title prefix plus source marker or plan-first label to avoid broad matches.
 *
 * @param {{title: string, body?: string, labels?: string[]}} issue
 * @returns {boolean}
 */
function isAutoGeneratedDailyIssue(issue) {
  const normalizedTitle = normalizeComparableText(issue.title ?? "");
  const hasDailyTitlePrefix = AUTO_GENERATED_DAILY_TITLE_PREFIXES
    .map(normalizeComparableText)
    .some((prefix) => normalizedTitle.startsWith(prefix));

  if (!hasDailyTitlePrefix) {
    return false;
  }

  const normalizedBody = normalizeComparableText(issue.body ?? "");
  const hasGeneratedSource = AUTO_GENERATED_DAILY_SOURCE_MARKERS
    .map(normalizeComparableText)
    .some((marker) => normalizedBody.includes(marker));
  const hasPlanFirstLabel = issue.labels?.includes("plan-first") ?? false;

  return hasGeneratedSource || hasPlanFirstLabel;
}
...
function buildNoCandidateCandidateLines({ closeRecommendedTaskIssues, staleTaskIssues }) {
  ...
  return [
    ...
    "- 主な変更点: `.github/` 以下のワークフロー・テンプレート・プロンプトを現在の運用と照合し、不整合や欠落を見つけて候補 issue（task 以外）を 1 件起票する。",
    "- 必要なテスト: 候補 issue を立てた後、日次プラン生成で候補として正しく表示されることを確認する。",
